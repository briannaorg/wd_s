...
function {%= prefix %}_font_url() {
	...
	*/
	$roboto = _x( 'on', 'Roboto font: on or off', '{%= prefix %}' );
	$open_sans = _x( 'on', 'Open Sans font: on or off', '{%= prefix %}' );
...
function {%= prefix %}_scripts() {
	...
	wp_enqueue_style( '{%= prefix %}-style', get_stylesheet_directory_uri() . '/style' . $suffix . '.css', array(), $version );

	wp_enqueue_script( '{%= prefix %}-project', get_template_directory_uri() . '/js/project' . $suffix . '.js', array( 'jquery' ), $version, true );
...
if ( class_exists( 'WDS_Simple_Page_Builder' ) && version_compare( WDS_Simple_Page_Builder::VERSION, '1.6', '>=' ) ) :
	 ...
	 */
	function {%= prefix %}_enqueue_page_builder_scripts() {
	...
	add_action( 'wds_page_builder_after_load_parts', '{%= prefix %}_enqueue_page_builder_scripts' );
 ...
 */
function {%= prefix %}_include_svg_definitions() {
